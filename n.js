...
function draw() {
    ...
    var verts = [
    {x:-5.45, y:-1.175, z: 0, u:0, v:0},
    {x: 1.82, y:-1.06, z: 0, u:im_width, v:0},
    {x: 1.82, y: 1.06, z: 0, u:im_width, v:im_height},
    {x: -5.45, y: 0.77, z: 0, u:0, v:im_height}
    ];
    var tverts = [];
...
function init() {
    ...
    object_mat = jsgl.makeOrientationAffine(
    	...
    	{x:1, y:3, z:0}, {x:1, y:0, z:0}, {x:0, y:12.5, z:0});

    camera_mat = jsgl.makeOrientationAffine(
    	{x:0, y:2, z: 0.2 + target_distance}, {x:0, y:0, z:-2}, {x:0, y:1.5, z:0});
...
function image_loaded() {
  ...
  fs.readFile('input_model.png', function(err, squid){
  	if (err) throw err;
  	img = new Image;
  	img.src = squid;
  ...
  fs.readFile('single_cover.png', function(err, squid){
  	if (err) throw err;
  	imgs = new Image;
  	images.push(imgs);
  	imgs.src = 'single_cover.png';
  ...
  function get_json(){
  	...
  	var json_data = canvas.toDataURL();

  	var file = 'img_out.json'
  	jsonfile.writeFile(file, json_data, function (err) {
  		console.error(err);
  	});

  	var out = fs.createWriteStream('output_img.png')
